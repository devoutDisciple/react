...
class AddDialog extends React.Component{
    ...
    async componentDidMount() {
        console.log(this.antdFormStore.editData);
        let data = this.antdFormStore.editData;
        console.log(moment(data.registerTime));
        await this.props.form.setFieldsValue({
            age: data.age,
            name: data.name,
            phone: data.phone,
            sex: data.sex == '男' ? 'man' : 'woman',
            registerTime: moment(data.registerTime)
        });
    }

    handleOk() {
        this.antdFormStore.setEditDialogVisible(false);
    }
